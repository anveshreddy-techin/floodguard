        ...
        {alerts.map((a) => (
          ...
          >
            ...
            <div className="flex items-center gap-2 pt-1 text-[11px]">
              <Link
                href="/safety"
                className="btn-glow-cyan px-2.5 py-1 text-white rounded-lg font-bold flex items-center gap-1 transition"
              >
                <Compass className="w-3.5 h-3.5" /> GUIDANCE
              </Link>
              <Link
                href="/flight-recorder"
                className="px-2.5 py-1 bg-slate-900/90 hover:bg-slate-800 text-cyan-300 border border-slate-700 rounded-lg font-mono font-medium flex items-center gap-1 transition"
              >
                <Radio className="w-3.5 h-3.5" /> TRACE
              </Link>
              {!a.acknowledged && (
                <button
                  onClick={() => handleAcknowledge(a.id)}
                  className="px-2.5 py-1 bg-slate-900 hover:bg-slate-800 text-slate-300 border border-slate-700 rounded-lg font-medium ml-auto flex items-center gap-1 transition active:scale-95"
                >
