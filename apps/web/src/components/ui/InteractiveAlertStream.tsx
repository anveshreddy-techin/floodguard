...
export const InteractiveAlertStream: React.FC = () => {
  ...
  return (
    <div className="glass-panel-glow rounded-2xl p-4 space-y-3 shadow-2xl text-xs">
      <div className="flex items-center justify-between border-b border-slate-800 pb-2.5">
        <div className="flex items-center gap-2">
          <Bell className="w-4 h-4 text-orange-400 animate-bounce" />
          <span className="font-mono font-black text-slate-100 text-xs uppercase tracking-wider">
            INTERACTIVE ALERT STREAM
          </span>
        </div>
        <span className="text-[10px] font-mono bg-orange-950 text-orange-300 px-2.5 py-0.5 rounded-lg border border-orange-800 font-bold shadow-[0_0_10px_rgba(249,115,22,0.4)]">
          2 ACTIVE
        </span>
      </div>
      ...
      <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
        {alerts.map((a) => (
          ...
          >
            ...
            <div className="text-[11px] text-slate-300 leading-snug">{a.reason}</div>
            ...
            <div className="flex flex-wrap items-center gap-1.5 pt-1 text-[10px] sm:text-[11px]">
              <Link
                href="/safety"
                className="btn-glow-cyan px-2 py-0.5 sm:px-2.5 sm:py-1 text-white rounded-lg font-bold flex items-center gap-1 transition shrink-0 whitespace-nowrap active:scale-95"
              >
              ...
              <Link
                ...
                className="px-2 py-0.5 sm:px-2.5 sm:py-1 bg-slate-900/90 hover:bg-slate-800 text-cyan-300 border border-slate-700 rounded-lg font-mono font-medium flex items-center gap-1 transition shrink-0 whitespace-nowrap active:scale-95"
              >
              ...
              {!a.acknowledged && (
                <button
                  ...
                  className="px-2 py-0.5 sm:px-2.5 sm:py-1 bg-slate-900 hover:bg-slate-800 text-slate-300 border border-slate-700 rounded-lg font-medium ml-auto flex items-center gap-1 transition active:scale-95 shrink-0 whitespace-nowrap"
                >
