...
export const WeatherAlertCard: React.FC<WeatherAlertCardProps> = ({ alerts = [] }) => {
    ...
    return (
      <div className="bg-emerald-50 border border-emerald-200 p-4 rounded-2xl flex items-center gap-3 shadow-xs">
        ...
        <div className="text-xs text-emerald-950 font-sans">
          <strong className="text-emerald-900 font-bold text-sm block">Normal Conditions — No Active Weather Warnings</strong>
  ...
  return (
    <div className="space-y-3 font-sans">
      <div className="flex items-center gap-2 text-xs font-bold text-slate-800 tracking-tight">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        {alerts.map((alt) => {
          ...
          const cleanSev = alt.severity.charAt(0) + alt.severity.slice(1).toLowerCase();
          const cleanCat = alt.category.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, (c: string) => c.toUpperCase());
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div className="space-y-1">
                  <span className={`px-2.5 py-0.5 rounded-md text-xs font-bold border shadow-2xs inline-block ${style.badgeBg}`}>
                    {cleanSev} • {cleanCat}
                  </span>
                ...
                <div className="text-right shrink-0 bg-white/80 border border-slate-200 px-2.5 py-1 rounded-lg">
                  <span className="text-[11px] text-slate-500 block font-medium">Horizon:</span>
                  <span className="text-xs font-bold text-slate-900">+{alt.forecast_horizon_hours}h</span>
                </div>
              </div>

              {/* Triggering Variables Grid */}
              {alt.triggering_variables && (
                <div className="bg-white border border-slate-200 p-2.5 rounded-xl text-xs space-y-1 text-slate-700 shadow-2xs">
                  <div className="text-[11px] text-slate-500 font-semibold">Triggering Conditions:</div>
