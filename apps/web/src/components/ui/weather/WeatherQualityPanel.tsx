      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-200 pb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-slate-900 tracking-tight">
            Data Provider Quality & Telemetry Audit
          </h3>
        </div>
        <span className="text-xs font-semibold text-emerald-800 bg-emerald-50 px-2.5 py-1 rounded-lg border border-emerald-300 w-fit">
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <span className={`px-2 py-0.5 rounded text-xs font-bold ${
                ...
                {q.quality_grade.replace(/_/g, ' ')}
              </span>
            </div>

            <div className="grid grid-cols-2 gap-2 text-xs text-slate-700">
              <div className="p-2.5 rounded-lg bg-white border border-slate-200 shadow-2xs">
                <div className="text-[11px] text-slate-500 font-medium">Completeness</div>
                <div className="font-bold text-blue-700 text-sm">{q.completeness_pct}%</div>
              </div>
              <div className="p-2.5 rounded-lg bg-white border border-slate-200 shadow-2xs">
                <div className="text-[11px] text-slate-500 font-medium">Avg Latency</div>
                <div className="font-bold text-teal-700 text-sm">{q.latency_avg_ms} ms</div>
              </div>
              <div className="p-2.5 rounded-lg bg-white border border-slate-200 shadow-2xs">
                <div className="text-[11px] text-slate-500 font-medium">Freshness Rate</div>
                <div className="font-bold text-emerald-700 text-sm">{q.freshness_compliance_pct}%</div>
              </div>
              <div className="p-2.5 rounded-lg bg-white border border-slate-200 shadow-2xs">
                <div className="text-[11px] text-slate-500 font-medium">Spike Anomalies</div>
                <div className="font-bold text-amber-700 text-sm">{q.spike_anomaly_count}</div>
              </div>
            </div>
