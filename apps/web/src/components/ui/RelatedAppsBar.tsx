...
export const APP_HUBS: Record<AppHubKey, AppHub> = {
  OPERATIONS: {
    ...
    apps: [
      ...
      { id: 'safety', label: 'My Safety & Guidance', href: '/safety', icon: Compass, tag: 'EVAC HUD', badgeColor: 'green' },
    ],
  },
  GEOSPATIAL: {
    key: 'GEOSPATIAL',
    ...
    apps: [
      ...
      { id: 'village', label: 'Village Dossier', href: '/village/loc-uk-chamoli', icon: MapPin, tag: 'VILLAGES' },
    ],
  },
  METEOROLOGY: {
    key: 'METEOROLOGY',
    ...
    apps: [
      ...
      { id: 'ingestion', label: 'Ingestion Jobs Queue', href: '/ingestion', icon: RefreshCw, tag: 'JOBS' },
    ],
  },
  FORENSICS: {
    key: 'FORENSICS',
    ...
    apps: [
      ...
      { id: 'replay', label: 'Historical Replay', href: '/replay', icon: History, tag: 'STRICT' },
...
export const RelatedAppsBar: React.FC<RelatedAppsBarProps> = ({ activeAppId, hubKey, className = '' }) => {
  ...
  return (
    <div className={`bg-white border border-slate-200 shadow-xs rounded-2xl p-2 sm:p-2.5 flex flex-col lg:flex-row lg:items-center justify-between gap-2.5 select-none ${className}`}>
      {/* Left: Hub Title & Badge */}
      <div className="flex items-center gap-2 shrink-0">
        <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded-md bg-slate-100 text-slate-700 border border-slate-200 uppercase tracking-wider">
          {hub.badge}
        </span>
        ...
        {hub.apps.map((app) => {
          ...
          return (
            <Link
              key={app.id}
              href={app.href}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium font-sans shrink-0 transition-all active:scale-95 ${
                isActive
