...
export const CommandTimeline: React.FC<CommandTimelineProps> = ({
  ...
  return (
    <div className="bg-white/95 border-t border-slate-200 px-4 py-2 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs z-30 select-none backdrop-blur-md shadow-sm">
      {/* Player Controls & Scrubber */}
      <div className="flex items-center gap-2.5 w-full sm:w-auto">
        <button
          onClick={() => setIsPlaying(!isPlaying)}
          className={`px-3 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-bold flex items-center gap-1.5 text-[11px] font-mono shadow-sm transition active:scale-95 ${
        ...
        {/* Speed Multipliers */}
        <div className="flex items-center bg-slate-100 border border-slate-200 rounded-lg p-0.5 font-mono text-[10px]">
          {[1, 2, 5].map((s) => (
            <button
              key={s}
              onClick={() => setSpeed(s)}
        ...
        <button
          ...
          className="p-1.5 rounded-lg bg-slate-100 border border-slate-200 text-slate-600 hover:text-slate-900 transition active:scale-95 shadow-sm"
      ...
      {/* Timestamp */}
      <div className="flex items-center gap-1.5 font-mono text-[11px] text-slate-700 bg-slate-100 px-2.5 py-1 rounded-lg border border-slate-200">
