...
const SliderInput = ({ label, value, min, max, step = 1, unit, onChange, color = '#06b6d4' }: any) => (
  <div className="mb-3">
    ...
    <input type="range" min={min} max={max} step={step} value={value} onChange={e => onChange(+e.target.value)} className="w-full h-1.5 rounded-full cursor-pointer appearance-none bg-slate-200" style={{accentColor: color}} />
  </div>
);
        ...
        <main className="flex-1 min-h-0 overflow-y-auto p-3.5 sm:p-5 lg:p-6 pb-24 md:pb-6">
          ...
          <div className="border-b border-slate-200 pb-4 mb-6">
            <div className="flex flex-wrap gap-2 mb-2">
              <span className="chip bg-blue-50 text-blue-800 border border-blue-200">SIH26192 PROTOTYPE</span>
              <span className="chip bg-slate-100 text-slate-700 border border-slate-200">5-SOURCE FUSION</span>
              <span className="chip bg-slate-100 text-slate-700 border border-slate-200">ML INFERENCE ACTIVE</span>
              <span className="chip bg-blue-50 text-blue-800 border border-blue-200">RESEARCH_PROTOTYPE</span>
              <span className="chip bg-slate-100 text-slate-700 border border-slate-200">CSI: 0.9416</span>
            ...
            <p className="text-xs text-slate-600 mt-1">Hilly Regions Flash Flood & Landslide EWS — Problem Statement SIH26192</p>
          ...
          <div className="fp fp-operational bg-white border border-slate-200 rounded-2xl p-4 mb-6 shadow-xl relative overflow-hidden">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className={`p-2 rounded-xl ${isLiveMode ? 'bg-emerald-500/20 text-emerald-700 border border-emerald-500/40' : 'bg-blue-500/20 text-blue-400 border border-blue-500/30'}`}>
                  <Radio className={`w-5 h-5 ${isFetchingLive ? 'animate-spin' : isLiveMode ? 'animate-pulse' : ''}`} />
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <h2 className="text-sm font-bold text-slate-900 uppercase tracking-wider">Multi-Source Ingestion & Telemetry Bar</h2>
                    {isLiveMode ? (
                      <span className="bg-emerald-500/20 text-emerald-700 border border-emerald-500/40 text-[10px] font-mono font-bold px-2 py-0.5 rounded-full flex items-center gap-1">
                        <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping" /> SATELLITE & RIVER LIVE
                      </span>
                    ) : (
                      <span className="bg-slate-100 text-slate-700 text-slate-600 text-[10px] font-mono px-2 py-0.5 rounded-full">CALIBRATED BASELINE</span>
                    )}
                  </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-5 gap-1.5 w-full md:w-auto text-[10px] font-mono">
                <div className="bg-slate-50 px-2 py-1 rounded border border-emerald-500/40 text-emerald-800 text-center">
                  Rain: Open-Meteo <span className="text-[9px] text-emerald-700 font-bold">(LIVE)</span>
                </div>
                <div className="bg-slate-50 px-2 py-1 rounded border border-emerald-500/40 text-emerald-800 text-center">
                  River: GloFAS <span className="text-[9px] text-emerald-700 font-bold">(LIVE)</span>
                </div>
                <div className="bg-slate-50 px-2 py-1 rounded border border-blue-300 text-blue-800 text-center">
                  Slope: SHALe FoS <span className="text-[9px] text-blue-600 font-bold">(PHYSICS)</span>
                </div>
                <div className="bg-slate-50 px-2 py-1 rounded border border-amber-500/40 text-amber-800 text-center">
                  IMD / CWC <span className="text-[9px] text-amber-700 font-bold">(MOU REQ)</span>
                </div>
                <div className="bg-slate-50 px-2 py-1 rounded border border-blue-300 text-blue-800 text-center">
                  IoT Nodes <span className="text-[9px] text-blue-600 font-bold">(SIM_STREAM)</span>
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
                <button
                  ...
                  className="btn-primary text-xs font-bold px-3.5 py-2 rounded-xl flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white shadow-xs active:scale-95 transition disabled:opacity-50"
                >
                ...
                <button
                  ...
                  className="fp text-xs font-bold font-mono px-3 py-2 rounded-xl text-amber-800 border border-amber-500/30 hover:bg-amber-50 active:scale-95 transition flex items-center gap-1.5"
                ...
                <button
                  ...
                  className="fp text-xs font-bold font-mono px-3 py-2 rounded-xl text-blue-800 border border-blue-200 hover:bg-blue-50 active:scale-95 transition flex items-center gap-1.5"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
            {/* Pillar 1 */}
            <div className="fp fp-operational rounded-2xl p-4 border-l-4 border-blue-500 bg-white">
            ...
            {/* Pillar 2 */}
            <div className="fp fp-operational rounded-2xl p-4 border-l-4 border-amber-500 bg-white">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                  <Waves className="w-4 h-4 text-amber-700" /> Pillar 2: Soil Moisture
                </h3>
                <span className="text-[10px] bg-amber-500/20 text-amber-800 px-2 py-0.5 rounded">TDR / API</span>
              </div>
              <SliderInput label="Saturation Index" value={soilSat} min={0} max={1} step={0.01} unit="" onChange={setSoilSat} color="#f59e0b" />
              <div className="text-xs text-slate-600 mt-2">Derived 7-day API: <span className="text-amber-700 font-mono">{(soilSat * 300).toFixed(1)} mm</span></div>
            </div>

            {/* Pillar 3 */}
            <div className="fp fp-operational rounded-2xl p-4 border-l-4 border-purple-500 bg-white">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                  <Mountain className="w-4 h-4 text-purple-700" /> Pillar 3: Slope Stability
                </h3>
                <span className="text-[10px] bg-purple-500/20 text-purple-800 px-2 py-0.5 rounded">Physics Model</span>
              </div>
              ...
              <div className="flex gap-4 mt-3">
                <div className="flex-1 bg-slate-50 p-2 rounded-lg text-center">
                  ...
                  <div className="text-[9px] mt-0.5 text-slate-600">{computed.fosStatus}</div>
                </div>
                <div className="flex-1 bg-slate-50 p-2 rounded-lg text-center">
                  <div className="text-[10px] text-slate-500 uppercase">TWI Index</div>
                  <div className="font-mono font-bold text-purple-700">{computed.twi.toFixed(2)}</div>
                  <div className="text-[9px] mt-0.5 text-slate-600">SATURATION RISK</div>
                </div>
              </div>
            </div>

            {/* Pillar 4 */}
            <div className="fp fp-operational rounded-2xl p-4 border-l-4 border-rose-500 bg-white">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                  <BarChart3 className="w-4 h-4 text-red-600" /> Pillar 4: Historical Inventory
                </h3>
                <span className="text-[10px] bg-rose-500/20 text-red-700 px-2 py-0.5 rounded">GSI / NRSC</span>
              </div>
              <div className="flex flex-wrap gap-2 mb-3">
                {PRESETS.map(p => (
                  <button key={p.label} onClick={() => { setGsiSusc(p.value); setSelectedPreset(p.label); }}
                    className={`text-[10px] px-2 py-1 rounded ${selectedPreset === p.label ? 'bg-rose-500 text-slate-900' : 'bg-slate-100 text-slate-700 text-slate-700 hover:bg-slate-700'}`}>
                    {p.label}
                  </button>
                ))}
              </div>
            ...
            {/* Pillar 5 */}
            <div className="fp fp-operational rounded-2xl p-4 border-l-4 border-blue-600 bg-white lg:col-span-2">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                  <Activity className="w-4 h-4 text-blue-600" /> Pillar 5: Real-Time IoT Telemetry
                </h3>
                <span className="text-[10px] bg-blue-50 text-blue-700 px-2 py-0.5 rounded border border-blue-200">Sensors</span>
              </div>
          ...
          {/* Live HUD */}
          <div className="fp fp-operational bg-white rounded-2xl p-6 mb-6 border border-slate-200 relative overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row gap-6 items-center justify-between relative z-10">
              <div className="flex items-center gap-6">
                ...
                <div>
                  ...
                  <div className="text-slate-700 text-sm font-mono flex items-center gap-2">
                    <Clock className="w-4 h-4 text-blue-600" />
                    {computed.alert === 'RED' ? 'IMMINENT — EVACUATE NOW' : `${computed.leadTime} MIN ACTIONABLE LEAD TIME`}
                  </div>
                </div>
              </div>
              <div className="flex-1 max-w-sm w-full bg-slate-50 p-4 rounded-xl border border-slate-200">
                <h4 className="text-[10px] text-slate-500 uppercase mb-2">NDRF Operational Directive</h4>
                <p className="text-sm text-slate-700 font-medium mb-3">{alertData.action}</p>
                <div className="flex justify-between items-center border-t border-slate-200 pt-2 mt-2">
                  <select value={selectedVillage} onChange={e => setSelectedVillage(e.target.value)} className="bg-white text-slate-900 border border-slate-200 text-xs text-slate-900 rounded p-1 outline-none">
                    {Object.keys(VILLAGES).map(k => <option key={k} value={k}>{VILLAGES[k].name}</option>)}
                  </select>
                  <span className="text-[10px] text-blue-600 font-medium">{villageInfo.battalion}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Attribution */}
          <div className="fp fp-operational rounded-2xl p-5 border border-slate-200 mb-6 bg-white">
            <h3 className="text-sm font-bold text-slate-900 mb-4 flex items-center gap-2">
              <Target className="w-4 h-4 text-slate-600" /> Multi-Source Risk Attribution
            </h3>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-2">
              {computed.attribution.map(a => (
                <div key={a.label} className="flex items-center gap-1.5">
                  ...
                  <span className="text-[9px] text-slate-600 uppercase truncate">{a.label} <span className="text-slate-900 font-mono">{a.pct.toFixed(1)}%</span></span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* ML Table */}
          <div className="fp fp-operational rounded-2xl p-5 border border-slate-200 bg-white overflow-x-auto">
            <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
              <h3 className="text-sm font-bold text-slate-900 flex items-center gap-2">
                <Cpu className="w-4 h-4 text-slate-600" /> ML Architecture Performance (Kedarnath/Wayanad Holdout)
              </h3>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {mlMetrics?.dataset_stats && (
                  <span className="text-[10px] font-mono px-2.5 py-1 rounded-full border font-semibold bg-slate-100 text-slate-700 text-slate-600 border-slate-200">
            ...
            {mlMetrics?.dataset_stats?.note && (
              <p className="text-[10px] text-amber-700/80 bg-amber-500/10 border border-amber-500/20 rounded px-3 py-2 mb-3 font-mono leading-relaxed">
                ⚠ {mlMetrics.dataset_stats.note}
              </p>
            )}
              ...
              <tbody className="text-slate-700 font-mono">
                {(() => {
                  ...
                  const dtBadge = (dt: string | undefined) => dt === 'REAL'
                    ? <span className="bg-emerald-500/20 text-emerald-700 border border-emerald-500/30 px-1.5 py-0.5 rounded text-[9px]">REAL</span>
                    : dt === 'UNKNOWN' || !dt
                      ? <span className="bg-slate-100 text-slate-700 text-slate-500 px-1.5 py-0.5 rounded text-[9px]">—</span>
                      : <span className="bg-amber-500/20 text-amber-700 border border-amber-500/30 px-1.5 py-0.5 rounded text-[9px]">{dt}</span>;
                  return (<>
                    <tr className="border-b border-slate-200">
                      ...
                      <td><span className="bg-slate-100 text-slate-700 text-slate-600 px-2 py-0.5 rounded text-[10px]">{tierA.status ?? 'RESEARCH_VALIDATED'}</span></td>
                    </tr>
                    <tr className="border-b border-slate-200">
                      ...
                      <td><span className="bg-slate-100 text-slate-700 text-slate-600 px-2 py-0.5 rounded text-[10px]">{tierB.status ?? 'RESEARCH_VALIDATED'}</span></td>
                    </tr>
                    <tr className="bg-cyan-950/20 border-b border-slate-200">
                      <td className="py-3 pl-2 text-blue-600 font-bold">Tier C RF Ensemble</td>
                      <td className="text-blue-600 font-bold">{fmt(tierC.pr_auc)}</td>
                      <td className="text-blue-600 font-bold">{fmt(tierC.roc_auc)}</td>
                      <td className="text-blue-600 font-bold">{fmt(tierC.csi)}</td>
                      ...
                      <td><span className="bg-blue-50 text-blue-800 border border-blue-200 px-2 py-0.5 rounded text-[10px]">{tierC.status ?? 'RESEARCH_PROTOTYPE'}</span></td>
                    </tr>
                    <tr>
                      ...
                      <td><span className="bg-slate-100 text-slate-700 text-slate-500 px-2 py-0.5 rounded text-[10px]">SUPPLEMENT</span></td>
                    </tr>
                  </>);
                })()}
              </tbody>
          ...
          {/* ══════════════════════════════════════════════════════════════════════════
              ...
              ══════════════════════════════════════════════════════════════════════════ */}
          <div className="mt-8 fp fp-operational rounded-2xl p-6 border border-slate-200 bg-white shadow-xl">
            {/* Header & Badges */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-slate-200">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <Award className="w-5 h-5 text-amber-700" />
                  <h2 className="text-lg font-bold text-slate-900 tracking-wide">Model Generalization Benchmark & Holdout Suite</h2>
                </div>
              ...
              <div className="flex flex-wrap gap-2 items-center">
                <span className="bg-amber-500/15 text-amber-800 border border-amber-500/30 text-[10px] font-mono px-2.5 py-1 rounded-full flex items-center gap-1.5 font-semibold">
                  <ShieldAlert className="w-3 h-3" /> LEVEL: BENCHMARKED_MODEL
                </span>
                <span className="bg-emerald-500/15 text-emerald-800 border border-emerald-500/30 text-[10px] font-mono px-2.5 py-1 rounded-full flex items-center gap-1.5 font-semibold">
                  <CheckCircle2 className="w-3 h-3" /> LEAKAGE_FREE (0 SPATIAL OVERLAP)
                </span>
                <span className="bg-blue-50 text-blue-800 border border-blue-200 text-[10px] font-mono px-2.5 py-1 rounded-full font-semibold">
            ...
            <div className="mt-5 p-4 rounded-xl bg-amber-950/20 border border-amber-500/30 text-xs text-amber-200/90 leading-relaxed flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-slate-700">
                  FloodGuard allows users to compute features for <strong className="text-slate-900">any valid latitude/longitude coordinate</strong>. However, the system strictly separates <span className="text-blue-700 underline underline-offset-2">software calculation capability</span> from <span className="text-amber-800 underline underline-offset-2">scientific predictive validity</span>. A model is never claimed to be universally accurate simply because it can ingest features. Accuracy in trained river basins (Chamoli, Kullu, Teesta) does not imply validity in uncalibrated or unseen catchments (Kedarnath, Wayanad).
                </p>
              </div>
            </div>
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 rounded-xl bg-slate-50 border border-slate-200">
                <div className="text-[11px] font-mono uppercase text-slate-600 font-bold mb-2 flex items-center gap-1.5">
                  <Database className="w-3.5 h-3.5 text-blue-600" /> Trained Basins (5 Regions)
                </div>
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {['UK_CHAMOLI', 'HP_KULLU', 'SK_TEESTA', 'AS_CACHAR', 'MH_MAHABALESHWAR'].map(r => (
                    <span key={r} className="text-[10px] font-mono bg-cyan-950/50 text-blue-700 px-2 py-0.5 rounded border border-cyan-800/40">
                      {r}
                    </span>
                  ))}
                </div>
              ...
              <div className="p-4 rounded-xl bg-slate-50 border border-slate-200">
                <div className="text-[11px] font-mono uppercase text-slate-600 font-bold mb-2 flex items-center gap-1.5">
                  <GitBranch className="w-3.5 h-3.5 text-red-600" /> Unseen Test Holdouts (2 Basins)
                </div>
                <div className="flex flex-wrap gap-1.5 mb-3">
                  {['UK_KEDARNATH', 'KL_WAYANAD'].map(r => (
                    <span key={r} className="text-[10px] font-mono bg-rose-950/50 text-red-700 px-2 py-0.5 rounded border border-rose-800/40 font-bold">
                      {r} (HELD OUT)
                    </span>
                  ))}
                </div>
              ...
              <div className="p-4 rounded-xl bg-slate-50 border border-slate-200">
                <div className="text-[11px] font-mono uppercase text-slate-600 font-bold mb-2 flex items-center gap-1.5">
                  <FileCheck className="w-3.5 h-3.5 text-emerald-700" /> Data Leakage Audit
                </div>
                <div className="space-y-1 text-xs">
                  <div className="flex justify-between">
                    <span className="text-slate-600">Spatial Basin Overlap:</span>
                    <span className="text-emerald-700 font-mono font-bold">0 Basins (0%)</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-600">Temporal Causality:</span>
                    <span className="text-emerald-700 font-mono font-bold">STRICT_CAUSAL</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-600">Audit Status:</span>
                    <span className="text-emerald-700 font-mono font-bold">LEAKAGE_FREE</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6 p-4 rounded-xl bg-slate-50 border border-slate-200">
              <h3 className="text-xs font-mono uppercase font-bold text-slate-700 mb-3 flex items-center gap-2">
                <BarChart3 className="w-4 h-4 text-blue-600" /> Geographic Holdout Validation Results (Unseen Basins Matrix)
              </h3>
              <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
                <div className="p-3 bg-white rounded-lg border border-slate-200 text-center">
                  <div className="text-[10px] text-slate-600 uppercase font-mono">ROC-AUC</div>
                  <div className="text-lg font-bold text-blue-600 font-mono mt-0.5">0.9927</div>
                </div>
                <div className="p-3 bg-white rounded-lg border border-slate-200 text-center">
                  <div className="text-[10px] text-slate-600 uppercase font-mono">PR-AUC</div>
                  <div className="text-lg font-bold text-blue-600 font-mono mt-0.5">0.8037</div>
                </div>
                <div className="p-3 bg-white rounded-lg border border-slate-200 text-center">
                  <div className="text-[10px] text-slate-600 uppercase font-mono">CSI (Threat Score)</div>
                  <div className="text-lg font-bold text-emerald-700 font-mono mt-0.5">0.5455</div>
                </div>
                <div className="p-3 bg-white rounded-lg border border-slate-200 text-center">
                  <div className="text-[10px] text-slate-600 uppercase font-mono">POD (Hit Rate)</div>
                  <div className="text-lg font-bold text-emerald-700 font-mono mt-0.5">1.0000</div>
                </div>
                <div className="p-3 bg-white rounded-lg border border-slate-200 text-center">
                  <div className="text-[10px] text-slate-600 uppercase font-mono">FAR (False Alarm)</div>
                  <div className="text-lg font-bold text-amber-700 font-mono mt-0.5">0.4545</div>
                </div>
                <div className="p-3 bg-white rounded-lg border border-slate-200 text-center">
                  <div className="text-[10px] text-slate-600 uppercase font-mono">Brier Score</div>
                  <div className="text-lg font-bold text-slate-200 font-mono mt-0.5">0.0225</div>
                </div>
                <div className="p-3 bg-white rounded-lg border border-slate-200 text-center">
                  <div className="text-[10px] text-slate-600 uppercase font-mono">Calibration</div>
                  <div className="text-lg font-bold text-blue-700 font-mono mt-0.5">0.654</div>
                </div>
              </div>
            </div>
            ...
            <div className="mt-6">
              <h3 className="text-xs font-mono uppercase font-bold text-slate-700 mb-3 flex items-center gap-2">
                <Target className="w-4 h-4 text-red-600" /> Per-Event Performance on Verified Historical Disasters
              </h3>
              <div className="overflow-x-auto rounded-xl border border-slate-200 bg-slate-50">
                <table className="w-full text-left text-xs whitespace-nowrap">
                  <thead>
                    <tr className="text-slate-600 uppercase font-mono text-[10px] border-b border-slate-200 bg-slate-50">
                  ...
                  <tbody className="divide-y divide-slate-800/60 text-slate-700 font-mono">
                    <tr className="hover:bg-slate-50">
                      <td className="py-3 px-3 font-semibold text-slate-900">2013 Kedarnath Cloudburst</td>
                      <td className="py-3 px-3 text-slate-600">Mandakini (UK)</td>
                      <td className="py-3 px-3 text-slate-700">Extreme Rain (350mm) + Moraine Breach</td>
                      <td className="py-3 px-3 text-[10px] text-slate-600">NDMA / IMD Gridded</td>
                      <td className="py-3 px-3 text-center text-blue-600">0.5455</td>
                      <td className="py-3 px-3 text-center text-emerald-700 font-bold">1.0000</td>
                      <td className="py-3 px-3 text-center text-amber-700">0.4545</td>
                      <td className="py-3 px-3 text-center text-blue-700">0.8333</td>
                      <td className="py-3 px-3 text-right pr-4">
                        <span className="bg-emerald-500/20 text-emerald-700 border border-emerald-500/30 text-[10px] px-2 py-0.5 rounded font-bold">
                          DETECTED
                        </span>
                      </td>
                    </tr>
                    <tr className="hover:bg-slate-50 bg-rose-950/10">
                      <td className="py-3 px-3 font-semibold text-slate-900">2021 Chamoli Rock-Ice Surge</td>
                      <td className="py-3 px-3 text-slate-600">Rishiganga / Alaknanda (UK)</td>
                      <td className="py-3 px-3 text-red-700">Glacial Rock Avalanche (ZERO Rain Trigger)</td>
                      <td className="py-3 px-3 text-[10px] text-slate-600">NIDM / CWC Joshimath</td>
                      <td className="py-3 px-3 text-center text-slate-600">0.0000</td>
                      <td className="py-3 px-3 text-center text-slate-600 font-bold">0.0000</td>
                      <td className="py-3 px-3 text-center text-slate-600">0.0000</td>
                      <td className="py-3 px-3 text-center text-slate-600">0.3333</td>
                      <td className="py-3 px-3 text-right pr-4">
                        <span className="bg-amber-500/20 text-amber-700 border border-amber-500/30 text-[10px] px-2 py-0.5 rounded font-bold" title="No rainfall observed; requires physical sensor detector">
                          PHYSICAL SENSOR REQUIRED
                        </span>
                      </td>
                    </tr>
                    <tr className="hover:bg-slate-50">
                      <td className="py-3 px-3 font-semibold text-slate-900">2023 Kullu Beas Surge</td>
                      <td className="py-3 px-3 text-slate-600">Beas Basin (HP)</td>
                      <td className="py-3 px-3 text-slate-700">Multi-Day Convergence (280mm/48h)</td>
                      <td className="py-3 px-3 text-[10px] text-slate-600">IMD AWS / CWC Thalout</td>
                      <td className="py-3 px-3 text-center text-blue-600">0.4545</td>
                      <td className="py-3 px-3 text-center text-emerald-700 font-bold">0.8333</td>
                      <td className="py-3 px-3 text-center text-amber-700">0.5000</td>
                      <td className="py-3 px-3 text-center text-blue-700">0.4722</td>
                      <td className="py-3 px-3 text-right pr-4">
                        <span className="bg-emerald-500/20 text-emerald-700 border border-emerald-500/30 text-[10px] px-2 py-0.5 rounded font-bold">
                          DETECTED
                        </span>
                    ...
                    <tr className="hover:bg-slate-50">
                      <td className="py-3 px-3 font-semibold text-slate-900">2023 Sikkim South Lhonak GLOF</td>
                      <td className="py-3 px-3 text-slate-600">Teesta Basin (SK)</td>
                      <td className="py-3 px-3 text-slate-700">Lateral Moraine Failure + Dam Breach</td>
                      <td className="py-3 px-3 text-[10px] text-slate-600">ISRO NRSC / Sikkim SDMA</td>
                      <td className="py-3 px-3 text-center text-blue-600 font-bold">0.6667</td>
                      <td className="py-3 px-3 text-center text-emerald-700 font-bold">1.0000</td>
                      <td className="py-3 px-3 text-center text-amber-700">0.3333</td>
                      <td className="py-3 px-3 text-center text-blue-700">0.6389</td>
                      <td className="py-3 px-3 text-right pr-4">
                        <span className="bg-emerald-500/20 text-emerald-700 border border-emerald-500/30 text-[10px] px-2 py-0.5 rounded font-bold">
                          DETECTED
                        </span>
                    ...
                    <tr className="hover:bg-slate-50">
                      <td className="py-3 px-3 font-semibold text-slate-900">2024 Wayanad Meppadi Debris Flow</td>
                      <td className="py-3 px-3 text-slate-600">Chaliyar Basin (KL)</td>
                      <td className="py-3 px-3 text-slate-700">Orographic Inundation (572mm/48h)</td>
                      <td className="py-3 px-3 text-[10px] text-slate-600">IMD Kozhikode / KSDMA</td>
                      <td className="py-3 px-3 text-center text-blue-600">0.5455</td>
                      <td className="py-3 px-3 text-center text-emerald-700 font-bold">1.0000</td>
                      <td className="py-3 px-3 text-center text-amber-700">0.4545</td>
                      <td className="py-3 px-3 text-center text-blue-700 font-bold">0.9583</td>
                      <td className="py-3 px-3 text-right pr-4">
                        <span className="bg-emerald-500/20 text-emerald-700 border border-emerald-500/30 text-[10px] px-2 py-0.5 rounded font-bold">
                          DETECTED
                        </span>
            ...
            <div className="mt-8 pt-6 border-t border-slate-200">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl">
                  <div className="text-[10px] font-mono text-blue-600 font-bold mb-1">1. DATA COVERAGE %</div>
                  <p className="text-[11px] text-slate-700">
                    Proximity to live IMD AWS, CWC gauges, and telemetry sensors. Zero live sensors = demo fallback.
                  </p>
                </div>
                <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl">
                  <div className="text-[10px] font-mono text-blue-600 font-bold mb-1">2. FEATURE COMPLETENESS %</div>
                  <p className="text-[11px] text-slate-700">
                    Fraction of required 25 hydrometeorological features available without defaulting or imputation.
                  </p>
                </div>
                <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl">
                  <div className="text-[10px] font-mono text-blue-600 font-bold mb-1">3. TRAINING COVERAGE %</div>
                  <p className="text-[11px] text-slate-700">
                    Inverse distance metric to the 5 baseline trained Himalayan and Western Ghats training corridors.
                  </p>
                </div>
                <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl">
                  <div className="text-[10px] font-mono text-blue-600 font-bold mb-1">4. VALIDATION COVERAGE %</div>
                  <p className="text-[11px] text-slate-700">
                    Degree of historical backtest verification within the same watershed or geological classification.
                  </p>
                </div>
                <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl">
                  <div className="text-[10px] font-mono text-amber-700 font-bold mb-1">5. MODEL APPLICABILITY %</div>
                  <p className="text-[11px] text-slate-700">
                    Physiographic matching score (steep slope vs alluvial plain) determining appropriate model routing.
                  </p>
                </div>
                <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl">
                  <div className="text-[10px] font-mono text-red-600 font-bold mb-1">6. OUT-OF-DISTRIBUTION (OOD)</div>
                  <p className="text-[11px] text-slate-700">
                    Feature Mahalanobis distance from training distribution. High OOD triggers mandatory confidence penalties.
                  </p>
                </div>
                <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl">
                  <div className="text-[10px] font-mono text-amber-700 font-bold mb-1">7. EPISTEMIC UNCERTAINTY</div>
                  <p className="text-[11px] text-slate-700">
                    Variance among decision trees and physics equations representing lack of domain calibration.
                  </p>
                </div>
                <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl">
                  <div className="text-[10px] font-mono text-emerald-700 font-bold mb-1">8. PREDICTION ELIGIBILITY</div>
                  <p className="text-[11px] text-slate-700">
                    Hierarchical state gate: <code className="text-emerald-800">VALIDATED</code> | <code className="text-blue-700">LIMITED</code> | <code className="text-amber-800">OOD</code> | <code className="text-red-700">NOT_ELIGIBLE</code>.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="mt-8 pt-6 border-t border-slate-200">
              <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider mb-2 flex items-center gap-2">
                <Shield className="w-4 h-4 text-emerald-700" /> Operational Validation Maturity Ladder
              </h3>
              <p className="text-xs text-slate-600 mb-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-5 gap-2">
                <div className="p-3 rounded-lg bg-slate-50 border border-slate-200 opacity-60">
                  <div className="text-[10px] font-mono text-slate-600 font-bold">STAGE 1</div>
                  <div className="text-xs font-bold text-slate-700 mt-1">RESEARCH_MODEL</div>
                  <p className="text-[10px] text-slate-500 mt-1">Algorithmic baseline formulation & synthetic lab benchmarks.</p>
                </div>
                <div className="p-3 rounded-lg bg-blue-50 border border-blue-300 relative">
                  <div className="absolute -top-2 right-2 bg-blue-600 text-white text-[8px] font-black px-1.5 py-0.5 rounded uppercase shadow-xs">
                    ACTIVE TIER
                  </div>
                  <div className="text-[10px] font-mono text-blue-600 font-bold">STAGE 2</div>
                  <div className="text-xs font-bold text-blue-900 mt-1">BENCHMARKED_MODEL</div>
                  <p className="text-[10px] text-slate-600 mt-1">Evaluated on real historical disaster holdouts with zero data leakage.</p>
                </div>
                <div className="p-3 rounded-lg bg-slate-50 border border-slate-200 opacity-60">
                  <div className="text-[10px] font-mono text-slate-600 font-bold">STAGE 3</div>
                  <div className="text-xs font-bold text-slate-700 mt-1">HISTORICALLY_BACKTESTED</div>
                  <p className="text-[10px] text-slate-500 mt-1">Multi-year synoptic hindcast replay across continuous seasonal records.</p>
                </div>
                <div className="p-3 rounded-lg bg-slate-50 border border-slate-200 opacity-60">
                  <div className="text-[10px] font-mono text-slate-600 font-bold">STAGE 4</div>
                  <div className="text-xs font-bold text-slate-700 mt-1">PILOT_MODEL</div>
                  <p className="text-[10px] text-slate-500 mt-1">Field trial deployed under official MoU with State Disaster Authority.</p>
                </div>
                <div className="p-3 rounded-lg bg-slate-50 border border-slate-200 opacity-60">
                  <div className="text-[10px] font-mono text-slate-600 font-bold">STAGE 5</div>
                  <div className="text-xs font-bold text-slate-700 mt-1">OPERATIONALLY_VALIDATED</div>
