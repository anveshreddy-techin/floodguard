  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden select-none bg-[#F0F4F8] text-slate-900">
      ...
      <div className="flex flex-1 min-h-0 relative">
        ...
        <main className="flex-1 min-h-0 p-3.5 sm:p-5 lg:p-6 max-w-7xl mx-auto space-y-6 pb-24 md:pb-6 overflow-y-auto">
          {/* Header Bar */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-200 pb-4 gap-3">
            <div>
              <div className="flex flex-wrap items-center gap-2">
                <span className="px-2.5 py-0.5 rounded-full bg-blue-100 text-blue-800 border border-blue-300 text-[10px] font-mono font-bold">SIH26192 HYPER-LOCAL OPERATIONAL DOSSIER</span>
              ...
              <h1 className="text-2xl font-black text-slate-900 flex items-center gap-2.5 mt-2">
                <MapPin className="w-6 h-6 text-blue-600" />
                {forecastData?.village || currentLoc.name}
              </h1>
              <p className="text-xs text-slate-600 mt-1 font-sans">
                {zoneLabels[currentLoc.zone] || currentLoc.zone} • River Basin: <strong className="text-blue-700">{currentLoc.region.split('(')[1]?.replace(')', '') || 'Headwaters'}</strong>
              </p>
            </div>
            <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="bg-white rounded-3xl p-5 sm:p-6 space-y-4 border border-slate-200 shadow-sm">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between border-b border-slate-200 pb-3 gap-3">
              ...
              <div className="flex items-center gap-3 bg-red-50 border border-red-200 px-4 py-2 rounded-2xl">
                <Clock className="w-5 h-5 text-red-600 animate-pulse" />
                <div>
                  <div className="text-[10px] font-mono text-red-700 font-bold uppercase">ACTIONABLE EVACUATION LEAD TIME</div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs font-mono">
              <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200">
                <span className="text-slate-500 font-bold block text-[10px]">POINT RISK SCORE</span>
                <span className="text-2xl font-black text-red-600">
                  {forecastData?.risk_score || currentLoc.riskScore}/100
                </span>
                <span className="text-[9px] text-slate-500 block mt-0.5">
                  CI 90%: <strong className="text-blue-700">{unc?.ci_90_formatted || `[${currentLoc.riskScore - 14} – ${currentLoc.riskScore + 14}]`}</strong>
                </span>
              </div>

              <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200">
                <span className="text-slate-500 font-bold block text-[10px]">CONSERVATIVE UPPER BOUND</span>
                <span className="text-2xl font-black text-amber-600">
                  {unc?.conservative_upper_bound || Math.min(100, currentLoc.riskScore + 14)}/100
                </span>
                <span className="text-[9px] text-red-600 block mt-0.5 font-bold">
                  {unc?.conservative_upper_bound >= 75 ? '⚠️ COMPULSORY EVACUATION' : 'MONITOR SURGE'}
                </span>
              </div>

              <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200">
                <span className="text-slate-500 font-bold block text-[10px]">TREE ENSEMBLE AGREEMENT</span>
              ...
              <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200">
                <span className="text-slate-500 font-bold block text-[10px]">WARDOVERVIEW</span>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
              {[
                { id: 1, title: '1. Rainfall QPE', icon: CloudRain, color: 'text-blue-600', weight: '35%' },
                { id: 2, title: '2. Soil Moisture', icon: Waves, color: 'text-amber-600', weight: '25%' },
                { id: 3, title: '3. Slope Stability', icon: Mountain, color: 'text-purple-600', weight: '20%' },
                { id: 4, title: '4. Landslide History', icon: History, color: 'text-red-600', weight: '10%' },
                { id: 5, title: '5. IoT Telemetry', icon: Radio, color: 'text-emerald-600', weight: '10%' },
              ].map((tab) => {
                const Icon = tab.icon;
                ...
                return (
                  <button
                    ...
                    className={`p-3 rounded-2xl border text-left transition active:scale-95 flex flex-col justify-between shadow-sm ${
                      active
                  ...
                  >
                    ...
                    <div className="mt-2">
                      <div className="text-xs font-mono font-bold text-slate-900">{tab.title}</div>
                      <div className="text-[9px] font-mono text-slate-500">Multi-Source Pillar</div>
                    </div>
                  </button>
                );
              })}
            ...
            <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm space-y-4">
              {selectedPillar === 1 && p?.pillar_1_rainfall && (
                <div className="space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-200 pb-3 gap-2">
                    <div>
                      <h3 className="text-xs font-mono font-bold text-blue-700 uppercase flex items-center gap-2">
                    ...
                    {p.pillar_1_rainfall.cloudburst_threshold_exceeded && (
                      <span className="px-2.5 py-1 rounded text-[10px] font-mono font-bold bg-red-100 text-red-700 border border-red-300 animate-pulse">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 text-xs font-mono">
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <span className="text-slate-500 font-bold block text-[10px]">15-MIN ACCUMULATION</span>
                      <span className="text-lg font-bold text-blue-700">{p.pillar_1_rainfall.rainfall_15m_mm} mm</span>
                      <span className="text-[9px] text-slate-500 block">Flash Trigger Gap</span>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <span className="text-slate-500 font-bold block text-[10px]">1-HOUR RAINFALL</span>
                      <span className="text-lg font-bold text-blue-700">{p.pillar_1_rainfall.rainfall_1h_mm} mm</span>
                      <span className="text-[9px] text-slate-500 block">Immediate Runoff</span>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <span className="text-slate-500 font-bold block text-[10px]">3-HOUR ACCUMULATION</span>
                      <span className="text-lg font-bold text-amber-700">{p.pillar_1_rainfall.rainfall_3h_mm} mm</span>
                      <span className="text-[9px] text-slate-500 block">Catchment Surge</span>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <span className="text-slate-500 font-bold block text-[10px]">24-HOUR TOTAL</span>
                      <span className="text-lg font-bold text-purple-700">{p.pillar_1_rainfall.rainfall_24h_mm} mm</span>
                      <span className="text-[9px] text-slate-500 block">Soil Saturation Base</span>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <span className="text-slate-500 font-bold block text-[10px]">PEAK INTENSITY</span>
                      <span className="text-lg font-bold text-red-600">{p.pillar_1_rainfall.rainfall_peak_intensity_mmph} mm/h</span>
                      <span className="text-[9px] text-slate-500 block">Convective Cell</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              {selectedPillar === 2 && p?.pillar_2_soil_moisture && (
                <div className="space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-200 pb-3 gap-2">
                    <div>
                      <h3 className="text-xs font-mono font-bold text-amber-700 uppercase flex items-center gap-2">
                    ...
                    <span className="px-2.5 py-1 rounded text-[10px] font-mono font-bold bg-amber-100 text-amber-800 border border-amber-300">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs font-mono">
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <span className="text-slate-500 font-bold block text-[10px]">VOLUMETRIC MOISTURE (VWC)</span>
                      <span className="text-lg font-bold text-amber-700">{p.pillar_2_soil_moisture.volumetric_moisture_pct}%</span>
                      <span className="text-[9px] text-slate-500 block">Calibrated TDR Probe</span>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <span className="text-slate-500 font-bold block text-[10px]">SATURATION RATIO (Sr)</span>
                      <span className="text-lg font-bold text-red-600">{(p.pillar_2_soil_moisture.soil_saturation_index * 100).toFixed(1)}%</span>
                      <span className="text-[9px] text-slate-500 block">Pore-Water Pressure</span>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <span className="text-slate-500 font-bold block text-[10px]">ANTECEDENT 7-DAY RAIN</span>
                      <span className="text-lg font-bold text-blue-700">{p.pillar_2_soil_moisture.antecedent_7d_mm} mm</span>
                      <span className="text-[9px] text-slate-500 block">Pre-Conditioning Load</span>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <span className="text-slate-500 font-bold block text-[10px]">EFFECTIVE COHESION (c&apos;)</span>
                      <span className="text-lg font-bold text-emerald-700">{p.pillar_2_soil_moisture.effective_cohesion_kpa} kPa</span>
                      <span className="text-[9px] text-slate-500 block">Shear Strength Residual</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              {selectedPillar === 3 && p?.pillar_3_slope_stability && (
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs font-mono">
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <span className="text-slate-500 font-bold block text-[10px]">FACTOR OF SAFETY (FoS)</span>
                      <span className="text-lg font-bold text-purple-700">{p.pillar_3_slope_stability.factor_of_safety_fos}</span>
                      <span className="text-[9px] text-slate-500 block">{p.pillar_3_slope_stability.factor_of_safety_fos < 1.0 ? 'Failure Imminent (<1.0)' : 'Stable (>1.3)'}</span>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <span className="text-slate-500 font-bold block text-[10px]">TERRAIN SLOPE (β)</span>
                      <span className="text-lg font-bold text-blue-700">{p.pillar_3_slope_stability.slope_degrees}°</span>
                      <span className="text-[9px] text-slate-500 block">DEM 30m Cell Incline</span>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <span className="text-slate-500 font-bold block text-[10px]">TOPOGRAPHIC WETNESS (TWI)</span>
                      <span className="text-lg font-bold text-amber-700">{p.pillar_3_slope_stability.topographic_wetness_index_twi}</span>
                      <span className="text-[9px] text-slate-500 block">ln(a / tan β) Convergence</span>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <span className="text-slate-500 font-bold block text-[10px]">CRITICAL RAIN THRESHOLD</span>
                      <span className="text-lg font-bold text-red-600">{p.pillar_3_slope_stability.critical_rainfall_threshold_mm} mm</span>
                      <span className="text-[9px] text-slate-500 block">Dynamic Failure Limit</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              {selectedPillar === 4 && p?.pillar_4_landslide_inventory && (
                <div className="space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-200 pb-3 gap-2">
                    <div>
                      <h3 className="text-xs font-mono font-bold text-red-700 uppercase flex items-center gap-2">
                    ...
                    <span className="px-2.5 py-1 rounded text-[10px] font-mono font-bold bg-red-100 text-red-700 border border-red-300">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs font-mono">
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <span className="text-slate-500 font-bold block text-[10px]">GSI NLSM SUSCEPTIBILITY INDEX</span>
                      <span className="text-lg font-bold text-red-600">{p.pillar_4_landslide_inventory.gsi_susceptibility_index} / 1.0</span>
                      <span className="text-[9px] text-slate-500 block">National Macro-Zonation</span>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <span className="text-slate-500 font-bold block text-[10px]">RECORDED BASIN EVENTS</span>
                      <span className="text-lg font-bold text-amber-700">{p.pillar_4_landslide_inventory.historical_events_in_basin} Past Events</span>
                      <span className="text-[9px] text-slate-500 block">GSI &amp; NRSC Catalog</span>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <span className="text-slate-500 font-bold block text-[10px]">LAST MAJOR RECORDED DISASTER</span>
                      <span className="text-sm font-bold text-blue-700 truncate block">{p.pillar_4_landslide_inventory.last_major_disaster}</span>
                      <span className="text-[9px] text-slate-500 block">Historical Validation Anchor</span>
                    </div>
                  </div>
                </div>
              )}
              ...
              {selectedPillar === 5 && p?.pillar_5_iot_telemetry && (
                <div className="space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-200 pb-3 gap-2">
                    <div>
                      <h3 className="text-xs font-mono font-bold text-emerald-800 uppercase flex items-center gap-2">
                    ...
                    <span className="px-2.5 py-1 rounded text-[10px] font-mono font-bold bg-emerald-100 text-emerald-800 border border-emerald-300 flex items-center gap-1.5">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs font-mono">
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <span className="text-slate-500 font-bold block text-[10px]">RIVER WATER LEVEL</span>
                      <span className="text-lg font-bold text-blue-700">{p.pillar_5_iot_telemetry.river_level_m} m</span>
                      <span className="text-[9px] text-slate-500 block">Ultrasonic Gauge</span>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <span className="text-slate-500 font-bold block text-[10px]">RATE OF RISE</span>
                      <span className="text-lg font-bold text-red-600">+{p.pillar_5_iot_telemetry.river_rate_of_rise_mph} m/h</span>
                      <span className="text-[9px] text-slate-500 block">Flash Threshold: &gt;0.3 m/h</span>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <span className="text-slate-500 font-bold block text-[10px]">GEOPHONE SEISMIC VIBRATION</span>
                      <span className="text-lg font-bold text-amber-700">{p.pillar_5_iot_telemetry.geophone_debris_vibration_db} dB</span>
                      <span className="text-[9px] text-slate-500 block">Debris Front Signature</span>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <span className="text-slate-500 font-bold block text-[10px]">CULVERT BACKPRESSURE RATIO</span>
                      <span className="text-lg font-bold text-purple-700">{p.pillar_5_iot_telemetry.culvert_backpressure_ratio}</span>
                      <span className="text-[9px] text-slate-500 block">Drainage Choke Head</span>
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {wards.map((ward) => {
                ...
                const borderClass = isCritical 
                  ...
                  : (isWarning ? 'border-amber-300 bg-amber-50/40' : 'border-slate-200 bg-slate-50');
                
                return (
                  <div key={ward.ward_id} className={`p-4 rounded-2xl border ${borderClass} space-y-3 shadow-sm`}>
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold ${
                        isCritical 
                          ...
                          : (isWarning ? 'bg-amber-100 text-amber-800 border border-amber-300' : 'bg-emerald-100 text-emerald-800 border border-emerald-300')
                      }`}>
                        {ward.alert_stage.replace('STAGE_', 'LVL ')}
                      </span>
                    </div>

                    <div className="grid grid-cols-3 gap-2 text-[11px] font-mono bg-white p-2.5 rounded-xl border border-slate-200">
                      <div>
                        <span className="text-slate-500 font-bold block text-[9px]">WARD RISK</span>
                        <span className={`font-black text-sm ${isCritical ? 'text-red-600' : (isWarning ? 'text-amber-600' : 'text-emerald-600')}`}>
                          {ward.risk_score}/100
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-500 font-bold block text-[9px]">LEAD TIME</span>
                        <span className="font-black text-slate-900 text-sm">
                          {ward.actionable_lead_time_minutes} min
                        </span>
                      </div>
                      <div>
                        <span className="text-slate-500 font-bold block text-[9px]">FACTOR SAFETY</span>
                        <span className={`font-bold text-sm ${ward.factor_of_safety_fos < 1.0 ? 'text-red-600' : 'text-emerald-600'}`}>
                    ...
                    <div className="space-y-1 text-[11px] font-mono">
                      <div className="text-slate-700 flex items-center justify-between">
                        <span className="text-slate-500">Evacuation Priority:</span>
                        <strong className={isCritical ? 'text-red-700 font-bold' : 'text-slate-800'}>{ward.evacuation_priority}</strong>
                      </div>
                      <div className="text-slate-700 flex items-center justify-between">
                        <span className="text-slate-500">Designated Shelter:</span>
                        <strong className="text-blue-700 truncate max-w-[190px]">{ward.designated_shelter}</strong>
                      </div>
                      <div className="text-slate-700 flex items-center justify-between">
                        <span className="text-slate-500">Safe Trail:</span>
                        <strong className="text-emerald-700 truncate max-w-[190px]">{ward.evacuation_trail}</strong>
                      </div>
                    </div>
                  </div>
                );
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="bg-white rounded-3xl p-5 space-y-4 border border-slate-200 shadow-sm">
              <h3 className="text-xs font-mono font-bold text-slate-900 uppercase tracking-wider flex items-center gap-2 border-b border-slate-200 pb-2">
              ...
              <div className="space-y-2.5 text-xs">
                {evac?.shelters?.primary && (
                  <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-between">
                    ...
                    <span className="px-2.5 py-1 rounded text-[10px] font-mono bg-emerald-100 text-emerald-800 border border-emerald-300 font-bold">
                      READY
                    </span>
                  </div>
                )}
                ...
                {evac?.shelters?.secondary && (
                  <div className="p-3.5 rounded-2xl bg-slate-50 border border-slate-200 flex items-center justify-between">
                    ...
                    <span className="px-2.5 py-1 rounded text-[10px] font-mono bg-amber-100 text-amber-800 border border-amber-300 font-bold">
                      STANDBY
                    </span>
                  </div>
                )}
                ...
                <div className="space-y-1.5">
                  {evac?.designated_trails?.map((t: any, idx: number) => (
                    <div key={idx} className="p-2.5 rounded-xl bg-slate-50 border border-slate-200 flex items-center justify-between text-xs">
                      <div>
                        <div className="font-mono font-bold text-slate-900">{t.trail_name}</div>
                        <div className="text-[10px] font-mono text-slate-500">{t.notes}</div>
                      </div>
                      <span className={`px-2 py-0.5 rounded text-[9px] font-mono font-bold ${
                        t.exposure === 'LOW' 
                          ? 'bg-emerald-100 text-emerald-800 border border-emerald-300' 
                          : (t.exposure === 'CRITICAL' ? 'bg-red-100 text-red-700 border border-red-300' : 'bg-amber-100 text-amber-800 border border-amber-300')
                      }`}>
                        {t.exposure} EXPOSURE
                      </span>
                    </div>
            ...
            <div className="bg-white rounded-3xl p-5 space-y-4 flex flex-col justify-between border border-red-200 shadow-sm">
              <div>
                <div className="flex items-center justify-between border-b border-slate-200 pb-2">
                  <h3 className="text-xs font-mono font-bold text-red-700 uppercase tracking-wider flex items-center gap-2">
                    <ShieldAlert className="w-4 h-4 text-red-600" />
                    NDRF COMMAND DIRECTIVE &amp; RESCUE DISPATCH
                  </h3>
                  <span className="text-[10px] font-mono text-red-700 font-bold bg-red-100 px-2 py-0.5 rounded border border-red-300">
                    SOP LEVEL 4
                  </span>
                </div>

                <div className="mt-3 p-3.5 rounded-2xl bg-red-50 border border-red-200 space-y-2">
                  ...
                  <div className="text-[10px] font-mono text-blue-700 pt-1 flex items-center gap-2 font-bold">
                    <Radio className="w-3.5 h-3.5 text-blue-600 animate-pulse" />
                    CMAS Cell Broadcast Warning: <strong className="text-slate-900">TRANSMITTED TO MONITORED TOWERS</strong>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-2 pt-2">
                <button
                  ...
                  className="btn-danger w-full py-3.5 rounded-2xl text-xs font-mono font-black text-white flex items-center justify-center gap-2 shadow-sm active:scale-95 transition"
                >
