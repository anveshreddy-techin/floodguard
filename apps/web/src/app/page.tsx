    ...
    <div className="flex flex-col h-screen overflow-hidden select-none bg-[#020714]">
      ...
      <div className="flex flex-1 min-h-0 relative">
        ...
        <main className="flex-1 relative min-h-0 overflow-hidden">
          ...
          {/* ── LEFT COLUMN: Stacked top-to-bottom, all at left-3, no overlaps ── */}
          <div className="hidden md:flex absolute top-3 left-3 z-[750] flex-col items-start gap-2 pointer-events-none">

            {/* ROLES button — top of left column */}
            <Link
              href="/role-workspace"
              className="pointer-events-auto fp fp-operational px-3.5 py-2 text-cyan-300 hover:text-white rounded-xl text-xs font-black font-mono flex items-center gap-1.5 shadow-2xl active:scale-95 transition border border-cyan-500/40"
              title="Open Role-Adaptive Mission Workspace for 10 Statutory Roles (Hotkey: W)"
            >
              <Users className="w-3.5 h-3.5 text-cyan-400" />
              <span>ROLES</span>
              <span className="text-[9px] px-1 py-0.5 rounded bg-cyan-950 text-cyan-400 font-bold border border-cyan-800">
                10
              </span>
            </Link>

            {/* Citizen Guidance HUD — stacks directly below ROLES button */}
            {isCitizen && (
              <div className="pointer-events-auto w-80 bg-slate-950/95 border border-cyan-500/40 rounded-2xl p-4 shadow-2xl backdrop-blur-xl space-y-3">
          ...
          {/* Mobile Citizen HUD — full-width below header (mobile only) */}
          {isCitizen && (
            <div className="md:hidden absolute top-3 left-3 right-3 z-[450] bg-slate-950/95 border border-cyan-500/40 rounded-2xl p-4 shadow-2xl backdrop-blur-xl space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-mono text-cyan-300 font-bold flex items-center gap-1.5">
                  <MapPin className="w-4 h-4 text-cyan-400" /> {t('what_to_do')}
                </span>
          ...
          <div className="hidden md:flex absolute bottom-20 left-3 z-[600] fp rounded-xl px-3 py-2 text-[10px] font-mono text-slate-400 items-center gap-2">
