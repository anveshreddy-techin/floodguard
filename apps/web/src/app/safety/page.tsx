...
export default function MySafetyPage() {
  ...
  const { selectedLocation, hierarchy, t } = useAdaptive();
  ...
  const locName = locationMode === 'BROWSER' ? 'My GPS Position' : selectedLocation.name;
  ...
  const exposureLevels: Array<{
  ...
  }> = [
    {
      ...
      msg: `Your coordinates in ${locState} are ~3.2 km away from active river channels. Continue routine monitoring.`,
    },
    {
      ...
      msg: `You are within 1.8 km of the modeled ${locName.split('/')[1] || locRegion.split('(')[0]} flood corridor. Stay alert and review candidate routes.`,
    },
    {
      status: 'INSIDE_HIGH_RISK_AREA',
      risk: 'HIGH',
      guidanceLvl: 2,
      title: `HIGH FLOOD RISK · ${locName.toUpperCase()}`,
      msg: `Your authorized location in ${locState} appears to be within a modeled high-risk area (${selectedLocation.primaryHazard}).`,
    },
    {
      status: 'INSIDE_EXTREME_RISK_AREA',
      risk: 'EXTREME',
      guidanceLvl: 3,
      title: `EXTREME SURGE RISK DETECTED · ${locState.toUpperCase()}`,
      msg: `Location is inside primary flood surge corridor. Move immediately to candidate higher ground at ${locName.split('/')[0]} Shelter.`,
    },
  ];
  ...
  const candidateRoutes = [
    {
      id: 'rt-1',
      name: `${locName.split('/')[0]} North Elevated Ridge Trail`,
      distance: '1.4 km',
      elevation: '+120m',
      status: sensorFailure ? 'ROUTE_SAFETY_NOT_VERIFIED' : 'CANDIDATE LOWER EXPOSURE',
      note: `Ascends higher ground above modeled ${locName.split('/')[1] || locRegion.split('(')[0]} flood contour. Surface safety not verified.`,
      blocked: false,
    },
    {
      id: 'rt-2',
      name: `${locName.split('/')[0]} Panchayat Relief Hall Connector`,
      distance: '2.1 km',
      elevation: '+85m',
      status: 'CANDIDATE ROUTE',
      note: 'Avoids low-lying drainage culvert and bridge bottlenecks.',
      blocked: false,
    },
    {
      id: 'rt-3',
      name: `${locName.split('/')[0]} Riverbed & Lowland Bypass Link`,
      distance: '0.9 km',
      elevation: '+5m',
      status: 'BLOCKED',
      note: `HIGH INUNDATION RISK — Intersects active ${selectedLocation.riverStage} surge path. Avoid completely.`,
      blocked: true,
    },
  ];
        ...
        <main className="flex-1 p-3.5 sm:p-5 lg:p-6 max-w-7xl mx-auto space-y-5 pb-24 md:pb-6 overflow-y-auto">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-800/80 pb-4 gap-3">
            <div>
              ...
              <p className="text-xs text-slate-400 mt-1 font-sans">
                📍 Location: <strong className="text-cyan-300">{locName}</strong> ({locState} · {locRegion.split('(')[0]})
              </p>
            </div>
          ...
          <div className="fp fp-critical rounded-2xl p-4 text-xs space-y-1.5 shadow-xl">
            <div className="flex items-center justify-between">
              <span className="font-black text-amber-300 uppercase tracking-wider flex items-center gap-2 font-mono text-xs">
                ...
                {locState.toUpperCase()} DISASTER MANAGEMENT DIRECTIVE
              </span>
              <span className="chip chip-live">AUTHORITY DIRECTIVE</span>
            </div>
            <p className="text-slate-200 leading-relaxed font-sans text-xs">
              &quot;Local administration in {locRegion.split('(')[0]} advises all residents in low-lying corridors of {locName} to stay alert, avoid crossing swollen streams, and monitor siren broadcasts.&quot;
            </p>
          </div>
          ...
          <div className="fp fp-operational rounded-3xl p-4 sm:p-5 shadow-2xl space-y-4">
            ...
            <EvacuationLeafletMap
              ...
              stateName={locState}
              shelterName={`${locName.split('/')[0]} Elevated Community Shelter`}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs font-mono">
              <div className="p-3 rounded-xl bg-slate-950/80 border border-slate-800">
                ...
                <div className="font-bold text-white mt-0.5 truncate">{locName.split('/')[0]} Shelter</div>
                <div className="text-[10px] text-emerald-400">+120m · 1.4 km away</div>
              </div>
              <div className="p-3 rounded-xl bg-slate-950/80 border border-slate-800">
                <div className="text-[10px] text-slate-400 uppercase">River / Water Stage</div>
                <div className="font-bold text-rose-400 mt-0.5 text-base truncate">{selectedLocation.riverStage}</div>
                <div className="text-[10px] text-rose-300">Rain: {selectedLocation.rainfall3h} in 3h</div>
              </div>
              <div className="p-3 rounded-xl bg-slate-950/80 border border-slate-800">
                <div className="text-[10px] text-slate-400 uppercase">Response Agency</div>
                <div className="font-bold text-purple-300 mt-0.5 truncate">{selectedLocation.authoritativeAgency.split('&')[0]}</div>
                <div className="text-[10px] text-purple-400">Boats Ready · ETA 8 min</div>
              </div>
              <div className="p-3 rounded-xl bg-slate-950/80 border border-slate-800">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs font-mono">
              <div className="fp rounded-xl p-4 space-y-2">
                <div className="text-cyan-300 font-bold uppercase tracking-wider text-[11px]">
                  WHY IS RISK ELEVATED IN {locState.toUpperCase()}?
                </div>
                <ul className="space-y-1.5 text-slate-300 text-xs">
                  <li className="flex items-center gap-2">
                    <span className="text-orange-400 font-bold">•</span> Heavy localized precipitation ({selectedLocation.rainfall3h} in 3h)
                  </li>
                  <li className="flex items-center gap-2">
                    <span className="text-orange-400 font-bold">•</span> River/drainage water level: {selectedLocation.riverStage}
                  </li>
                  <li className="flex items-center gap-2">
                    <span className="text-orange-400 font-bold">•</span> Saturated watershed slopes &amp; soil ({selectedLocation.soilMoisture})
                  </li>
                </ul>
              </div>
              <div className="fp rounded-xl p-4 space-y-2">
                <div className="text-slate-400 font-bold uppercase tracking-wider text-[11px]">
                  ROUTE INTEGRITY ({locRegion.split('(')[0]})
                </div>
                <div className="space-y-1.5">
                  ...
                  <div className="flex justify-between items-center text-[10px] text-slate-400 pt-1 border-t border-slate-800">
                    ...
                    <span className="text-cyan-300 font-bold">+{selectedLocation.leadTimeMinutes} MIN WINDOW</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div>
                <h3 className="text-sm font-black text-white flex items-center gap-2">
                  ...
                  CANDIDATE LOWER-EXPOSURE PATHS FOR {locName.toUpperCase()}
                </h3>
                <p className="text-[11px] text-slate-400 mt-0.5">
                  Route safety is NOT guaranteed. Evaluated against active {locState} flood boundaries and known bridge status.
                </p>
              </div>
          ...
          <div className="fp rounded-3xl p-6 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div>
              <h4 className="text-xs font-black text-white uppercase tracking-wider">
                Emergency Assistance &amp; Rescue Dispatch ({locState})
              </h4>
              <p className="text-xs text-slate-400 mt-0.5 font-sans">
                Share authorized coordinates with {locState} SDRF response team or call national emergency.
              </p>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                <span>🚨 DISPATCH RESCUE &amp; CALL {locState.toUpperCase()} HELPLINE</span>
              </button>
            </div>
          </div>
