      ...
      <div className="flex flex-1 min-h-0 relative">
        ...
        <main className="flex-1 min-h-0 overflow-y-auto p-3.5 sm:p-5 lg:p-6 pb-24 md:pb-6">
          <div className="max-w-7xl mx-auto space-y-5">
            <RelatedAppsBar activeAppId="events" />
          {/* Header */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-200 pb-4 gap-3">
            <div>
              <div className="flex items-center gap-2.5">
                <span className="px-2.5 py-0.5 rounded-full bg-purple-100 text-purple-800 border border-purple-300 text-xs font-semibold">AUTHORITATIVE CORPUS</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
              ...
              {filteredDossiers.map((ev) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full p-3.5 rounded-2xl text-left transition-all duration-300 flex flex-col justify-between space-y-1.5 bg-white shadow-sm border ${
                      isSelected
                        ? 'border-blue-500 ring-2 ring-blue-400/30 bg-blue-50/30 shadow-md'
                  ...
                  >
                    ...
                    <div className="font-bold text-slate-900 text-xs font-sans leading-snug">{ev.name}</div>
                    <div className="text-xs text-slate-500 font-sans flex items-center gap-1">
            ...
            <div className="lg:col-span-7 bg-white border border-slate-200 shadow-sm rounded-3xl p-5 sm:p-7 space-y-5 animate-slide-up">
              <div className="border-b border-slate-200 pb-4">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  <span className="text-xs font-sans text-purple-800 font-semibold uppercase bg-purple-50 px-2.5 py-0.5 rounded-md border border-purple-200">
                    {current.type}
                  </span>
                  <span className="text-slate-400">•</span>
                  <span className="text-xs font-sans text-blue-700 font-medium">{current.country}</span>
                  <span className="text-slate-400">•</span>
                  <span className="text-xs font-sans text-slate-500">{current.date}</span>
                </div>
                <h2 className="text-lg sm:text-xl font-bold font-sans text-slate-900 mt-2">{current.name}</h2>
              </div>

              {/* Primary Cause */}
              <div className="bg-slate-50 border border-slate-200 p-4 rounded-2xl space-y-1 text-xs">
                <span className="text-xs font-sans text-blue-800 font-bold uppercase flex items-center gap-1.5">
                  <Flame className="w-3.5 h-3.5 text-red-600" />
                  PRIMARY PHYSICAL CAUSE
                </span>
                <p className="text-slate-700 leading-relaxed font-sans text-xs">{current.primaryCause}</p>
              </div>

              {/* Physical Hazard Propagation Chain */}
              <div className="space-y-2">
                <span className="text-xs font-sans text-slate-700 font-bold uppercase tracking-wider block">
                  PHYSICAL HAZARD PROPAGATION SEQUENCE
                </span>
                <div className="space-y-2 text-xs">
                  {current.hazardChain.map((step, i) => (
                    <div key={i} className="bg-slate-50 border border-slate-200 p-3 rounded-xl text-slate-800 flex items-start gap-2.5 font-sans">
                      <span className="text-blue-700 font-bold shrink-0 bg-white px-2 py-0.5 rounded border border-blue-200 font-mono text-xs shadow-xs">
                        0{i + 1}
                      </span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs font-sans">
                <div className="bg-slate-50 border border-slate-200 p-3.5 rounded-2xl space-y-1">
                  <div className="text-slate-500 text-xs uppercase font-bold">OFFICIAL CASUALTIES (GAZETTE)</div>
                  <div className="text-red-700 font-bold text-xs">{current.officialCasualties}</div>
                </div>
                <div className="bg-slate-50 border border-slate-200 p-3.5 rounded-2xl space-y-1">
                  <div className="text-slate-500 text-xs uppercase font-bold">AUTHORITATIVE SOURCES</div>
                  <div className="text-blue-700 text-xs font-medium">{current.authoritativeSources.join(', ')}</div>
                </div>
              </div>

              {/* Key System Lessons Learned */}
              <div className="bg-slate-50 border border-slate-200 p-4 rounded-2xl space-y-2 text-xs">
                <span className="text-xs font-sans text-emerald-800 font-bold uppercase flex items-center gap-1.5">
                  <Award className="w-3.5 h-3.5 text-emerald-600" />
                  KEY LESSONS FOR REAL-TIME FLOOD ENGINES
                </span>
                <ul className="space-y-1 text-slate-700 list-disc list-inside font-sans text-xs">
